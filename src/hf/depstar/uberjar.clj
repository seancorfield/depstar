    ...
    (when (.exists pom)
      (let [pom-data    (:content (xml/parse pom))
            ...
            manifest    (str "Manifest-Version: 1.0\n"
                             ...
                             (when-not (= :thin (:jar options))
                               (str "Main-Class: "
                                    (or (:main-class options) "clojure.main")
                                    "\n")))
        ...
        (with-open [is (io/input-stream pom)]
          (when *verbose*
...
(defn uber-main
  ...
  (let [verbose (some #(#{"-v" "--verbose"} %) args)
        ...
        main-class (some #(when (#{"-m" "--main"} (first %))
                            (second %))
                         (partition 2 1 args))]
    (run (assoc opts :main-class main-class :no-pom no-pom :verbose verbose))))
